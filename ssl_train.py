...
def ModelTrain(train_df_path, val_df_path, path_image, ModelType, CriterionType, device, LR):
    ...
    train_df = train_df.filter(['Image Index', 'Finding Labels'], axis=1)
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        if epoch==0:
            ...
            train_ssl_df = train_df[0:800]

        else:
            print("pseudo labeling phases")
            ssl_df = pd.DataFrame({'Image Index': pseudo_image_idx,
                                         'Finding Labels': pseudo_labels
                                        })
            train_ssl_df = pd.concat([train_ssl_df, ssl_df], ignore_index=True)
            
            print("train set size: ", len(train_ssl_df))
        
        unlabeled_df = train_df[(epoch+1) * 800:(epoch+2) * 800]
    ...
    # -------------------------- Start of phase
        ...
        phase = 'pseudo_label'
        optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, model.parameters()), lr=LR)
        pseudo_image_idx, pseudo_labels, running_loss = BatchIterator(model=model, phase=phase, Data_loader=unlabeled_loader, criterion=criterion, optimizer=optimizer, device=device)
